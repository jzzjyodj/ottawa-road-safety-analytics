...
- Filter on geo_valid = TRUE only when the query returns lat or long columns or is explicitly a map-based or spatial query. Do not apply geo_valid = TRUE filtering to pure count or aggregation queries that do not use coordinates.
...
- When filtering by a date range that spans multiple years and months, always wrap the full date range condition in explicit parentheses. Example: WHERE ((accident_year = 2021 AND accident_month >= 3) OR (accident_year = 2022 AND accident_month <= 3))
- When using GROUP BY on a nullable text column such as location or geo_id, always add WHERE column IS NOT NULL before the GROUP BY clause to exclude null rows from aggregation results.
...
| geo_valid | BOOLEAN | True if coordinates are non-null, non-zero, and within Ottawa bounding box. Filter on geo_valid = TRUE only when returning lat/long or for map-based queries — not for count or aggregation queries. |
